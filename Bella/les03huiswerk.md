```
>>> [0,1,2,3]
[0, 1, 2, 3]
>>> reeks = [0,1,2,3]
>>> reeks
[0, 1, 2, 3]
>>> naam = "Bella"
>>> vakken = ["Media Design", "Photography", "Typography"]
>>> vakken
['Media Design', 'Photography', 'Typography']
>>> len (vakken)
3
>>> vakken.append("Graphic Design")
>>> len(vakken)
4
>>> vakken.pop()
'Graphic Design'
>>> len(vakken)
3
>>> print len(vakken)
3
>>> vakken
['Media Design', 'Photography', 'Typography']
>>> vakken = vakken + ["Graphic Design", "Philosophy"]
>>> len(vakken)
5
>>> vakken.pop90
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
AttributeError: 'list' object has no attribute 'pop90'
>>> vakken.pop()
'Philosophy'
>>> vakken.pop()
'Graphic Design'
>>> vakken.pop()
'Typography'
>>> vakken
['Media Design', 'Photography']
>>> vakken = vakken + ["Graphic Design", "Philosophy"]
>>> vakken
['Media Design', 'Photography', 'Graphic Design', 'Philosophy']
>>> del vakken[1]
>>> vakken
['Media Design', 'Graphic Design', 'Philosophy']
```
```
>>> huis = {'adres': "Molenstraat 9", 'huisdieren': 2, 'huur':420}
>>> huis
{'huur': 420, 'huisdieren': 2, 'adres': 'Molenstraat 9'}
>>> huis['huisdier']
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
KeyError: 'huisdier'
>>> huis['huisdieren']
2
>>> huis['huur']
420
>>> 
>>> def maalvijf(x):
...     return x * 5
... 
>>> maalvijf(2)
10
>>> huis['huisdieren']
2
>>> huis['huisdieren'] = 3
>>> huis['huisdieren']
3
>>> huis
{'huur': 420, 'huisdieren': 3, 'adres': 'Molenstraat 9'}
>>> ['huisdieren'] = ['Pimmmetje', 'Pimmetje'], 'adres':
  File "<stdin>", line 1
    ['huisdieren'] = ['Pimmmetje', 'Pimmetje'], 'adres':
                                                       ^
SyntaxError: invalid syntax
>>> huis
{'huur': 420, 'huisdieren': 3, 'adres': 'Molenstraat 9'}
>>> ['huisdieren'] = ['Pimmetje', 'Balisto'], 'adres': 'Molenstraat 9'}
  File "<stdin>", line 1
    ['huisdieren'] = ['Pimmetje', 'Balisto'], 'adres': 'Molenstraat 9'}
                                                     ^
SyntaxError: invalid syntax
>>> huis['huisdieren']
3
>>> huis
{'huur': 420, 'huisdieren': 3, 'adres': 'Molenstraat 9'}
>>> 
```